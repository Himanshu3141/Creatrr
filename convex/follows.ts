...
type FollowerSummary = {
  ...
  followsBack: boolean;
  postCount: number;
  lastPostAt: number | null | undefined;
};
...
export const toggleFollow = mutation({
  ...
  handler: async (ctx, args): Promise<ToggleFollowResponse> => {
    ...
    const existingFollow: FollowDoc | null = await ctx.db
      .query("follows")
      .filter((q) =>
        q.and(
...
export const getMyFollowers = query({
  ...
  handler: async (ctx, args): Promise<FollowerSummary[]> => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) {
      return [];
    }
    ...
    const followers = await Promise.all(
      follows.map(async (follow): Promise<FollowerSummary | null> => {
    ...
    return followers.filter(
      (user): user is FollowerSummary => user !== null
    );
  },
});
...
export const getMyFollowing = query({
  args: { limit: v.optional(v.number()) },
  handler: async (ctx, args): Promise<FollowingSummary[]> => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) {
      return [];
    }
    ...
    const following = await Promise.all(
      follows.map(async (follow): Promise<FollowingSummary | null> => {
    ...
    return following.filter(
      (user): user is FollowingSummary => user !== null
    );
  },
});