...
export default function ProfilePage({ params }: ProfilePageProps) {
  ...
  const {
    ...
    error: userError,
  } = useConvexQuery<PublicUser | null>(convexApi.users.getByUsername, {
    username,
  });

  // Get user's posts
  const { data: postsData, isLoading: postsLoading } =
    useConvexQuery<PublishedPostsResponse>(
      convexApi.public.getPublishedPostsByUsername,
      {
        username,
        limit: 20,
      }
  ...
  const { data: followerCount } = useConvexQuery(
    convexApi.follows.getFollowerCount,
    user ? { userId: user._id } : "skip"
  );

  // Check if current user is following this profile
  const { data: isFollowing } = useConvexQuery(
    convexApi.follows.isFollowing,
